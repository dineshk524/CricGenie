...
const GAP_MS = 1; // wait between one card finishing and the next starting
const RESET_ON_EXIT = true; // hide/reset when section leaves view so it can replay
...
const FEATURES: Feature[] = [
  {
    title: "Live Scores",
    desc: "Ball-by-ball commentary & instant updates.",
    icon: <span className="text-xl">📺</span>,
    glowFrom: "from-sky-400/30",
    glowTo: "to-emerald-400/30",
  },
  {
    title: "Smart Notifications",
    desc: "Auto alerts for wickets, milestones & results.",
    icon: <span className="text-xl">🔔</span>,
  ...
  {
    title: "Player Stats",
    desc: "Form, trends & deep performance analytics.",
    icon: <span className="text-xl">📊</span>,
  ...
  },
  {
    ...
    desc: "Fixtures across series & tournaments.",
    icon: <span className="text-xl">📅</span>,
    glowFrom: "from-sky-400/30",
  ...
  {
    title: "Team Analysis",
    desc: "Head-to-head & advanced team insights.",
    icon: <span className="text-xl">👥</span>,
    glowFrom: "from-purple-400/30",
    glowTo: "to-fuchsia-400/30",
  },
];
...
const FeaturesCriccGenie: React.FC = () => {
  ...
  // visibility state for each card (controls the drop)
  const [show, setShow] = useState<boolean[]>(() =>
    new Array(FEATURES.length).fill(false)
  );

  // sequentially reveal cards when in view
  useEffect(() => {
    const timers: number[] = [];

    if (inView) {
      // start hidden
      setShow(new Array(FEATURES.length).fill(false));

      // chain timers so each card waits for previous (duration + gap)
      FEATURES.forEach((_, i) => {
        ...
        }, i * (DURATION_MS + GAP_MS));
        timers.push(t);
      });
    } else if (RESET_ON_EXIT) {
      // reset when we leave the section (so it can replay)
      setShow(new Array(FEATURES.length).fill(false));
    }
  ...
  return (
    <section
      ref={sectionRef}
      className="relative z-10 bg-transparent px-6 py-16 text-white md:py-20 mt-10"
    >
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mx-auto mb-10 max-w-4xl text-center md:mb-12">
          <h2
            ...
            style={{
              ...transition,
              transform: inView ? "none" : `translateY(${DROP_PX}px)`,
              opacity: inView ? 1 : 0,
            }}
          >
            Everything{" "}
          ...
          {FEATURES.map((f, i) => {
            ...
            return (
              <article
                ...
                style={{
                  ...transition,
                  // each card drops from the top; becomes visible when its turn comes
              ...
              >
                {/* hover glow */}
                <div
                  className={`pointer-events-none absolute -inset-6 -z-10 rounded-[28px] bg-gradient-to-r ${f.glowFrom} ${f.glowTo} opacity-0 blur-3xl transition-opacity duration-500 group-hover:opacity-40`}
                />
                {/* sheen */}
